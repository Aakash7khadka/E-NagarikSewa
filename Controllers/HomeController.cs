    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var appointment_day = _db.Appointment.Where(s=>s.isCompleted==true).GroupBy(s => s.Date).Select(x=> new{Date= x.Key, AppointmentCount=x.Count() }).OrderByDescending(s=>s.AppointmentCount).ToList();
